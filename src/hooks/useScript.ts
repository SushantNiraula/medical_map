  ...
  useEffect(
    () => {
      ...
      if (!scriptNode) {
        ...
        if (script.attributes && Object.entries(script.attributes).length > 0) {
        ...
        const currentScriptStatus =
          scriptToAdd.status ?? cachedScriptStatus ?? "loading";

        switch (currentScriptStatus) {
          case "loading":
          case "ready":
            if (script.callbacks?.onLoadCallback)
              script.callbacks.onLoadCallback();
            break;
          case "error":
            if (script.callbacks?.onErrorCallback)
